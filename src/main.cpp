...
int main() {
  ...
  if (int err = glfwInit() == 0) {
    std::cerr << "Failed to initalize glfw with error code: " << err
              << std::endl;
    std::terminate();
  }
  ...
  // create the window
  window =
      glfwCreateWindow(1280, 720, "Space! I'm in space!", nullptr, nullptr);
  if (!window) {
  ...
  if (int err = glewInit() != GLEW_OK) {
    std::cerr << "Error initalizing glew: " << glewGetErrorString(err)
              << std::endl;
    std::terminate();
  }
